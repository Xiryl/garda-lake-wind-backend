...
const getDataFromUrl = (url) => new Promise(async (resolve, reject) => {
  ...
  await axios({
    ...
    if (res.data) {
      ...
      jsonData.wind.speed = Number(Number($(listTd[21]).text()) * Number(0.539957)).toFixed(1);
      jsonData.wind.avg10minutes = `${Number(Number($(listTd[23]).text().trim().split(' ')[1]) * Number(0.539957)).toFixed(1)}`;
