...
const getDataFromUrl = (url) => new Promise(async (resolve, reject) => {
  ...
  await axios({
    ...
    if (res.data) {
      ...
      jsonData.wind.speed = `${Number(Number($(listTd[58]).text().replace(',', '.')) * Number(0.539957)).toFixed(1)}`;
      jsonData.wind.direction = $(listTd[65]).text().replace(',', '.').split(' ')[1];
      jsonData.wind.avg10minutes = '';
      jsonData.wind.raff = `${Number(Number($(listTd[59]).text().replace(',', '.')) * Number(0.539957)).toFixed(1)}`;
